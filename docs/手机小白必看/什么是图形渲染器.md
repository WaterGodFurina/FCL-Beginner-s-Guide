...
- **为了性能（换图形 API 或重写逻辑）**：
  原版 OpenGL 的 CPU 开销较大，老旧设备容易卡顿。于是社区开发了 **[Sodium](https://github.com/CaffeineMC/sodium)**（用现代 OpenGL 重写渲染逻辑，大幅提升帧率），甚至出现了 **[VulkanMod](https://github.com/Collateral/VulkanMod)**（直接把图形 API 从 OpenGL 替换为 Vulkan，以降低 CPU 开销）。
